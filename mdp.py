...
from utils import orientations, turn_left, turn_right, vector_add
...
class GridMDP(MDP):
    ...
    def to_grid(self, mapping):
        ...
        return list(
            reversed([[mapping.get((x, y), None) for x in range(self.cols)]
                      for y in range(self.rows)]))
...
sequential_decision_environment = GridMDP(
    [[-0.04, -0.04, -0.04, +1], [-0.04, None, -0.04, -1],
     [-0.04, -0.04, -0.04, -0.04]],
    terminals=[(3, 2), (3, 1)])
...
class POMDP(MDP):
    ...
    def remove_dominated_plans(self, input_values):
        ...
        values = [
            val for action in input_values for val in input_values[action]
        ]
...
def pomdp_value_iteration(pomdp, epsilon=0.1):
    ...
    while True:
        ...
        if count > 10:
            if pomdp.max_difference(
                    U, prev_U) < epsilon * (1 - pomdp.gamma) / pomdp.gamma:
                return U
